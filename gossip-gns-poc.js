...
try {
  ...
  console.log(`🚫 Error setting up client: ${error.message}`);
  process.exit(1);
}
...
async function createHCSTopic() {
  try {
    const topicTx = new TopicCreateTransaction()
      .setAdminKey(PrivateKey.fromString(OPERATOR_KEY).publicKey) // Use operator key's public key
...
async function mintZeroPointID(accountId, name) {
  try {
    ...
    const tokenCreateTx = new TokenCreateTransaction()
      ...
      .setAdminKey(PrivateKey.fromString(OPERATOR_KEY).publicKey) // Use operator key's public key
      .setSupplyKey(PrivateKey.fromString(OPERATOR_KEY).publicKey) // Use operator key's public key
